...
const LoginFrom = props => {
    ...
    const handleLogin = async (e, dispatch) => {
        ...
        try {
            ...
            alert(
                `- Error on login, message: ${data} - Status code of ${status} - Status message of ${statusText}`
            )
            setLoading(false)
    ...
    return (
        <>
            {loading ? (
                ...
                <form
                    onSubmit={e => handleLogin(e, dispatch)}
                    style={{ margin: '0 5% 2rem 5%' }}
                >
                    <label htmlFor="name">Please insert your name</label>
                    <br></br>
                    <input
                        id="nameLogin"
                        type="text"
